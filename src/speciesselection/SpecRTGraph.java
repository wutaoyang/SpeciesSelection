...
public class SpecRTGraph {
    ...
    SpecRTGraph(){
        specNode=new ArrayList<Species>();
        rtNode=new ArrayList<ResourceType>();
    }
    ...
    */
     ...
     * @return A minimal constitute species tree with root 
     * specified by rootSpec; null if rootSpec is not a  species
     */
    
   
    
    public SpecTree getMinimalConstSpecTreeRootedAt(Species rootSpec){
        if(!this.containsSpecies(rootSpec)) return null;
     ...
     * @return The first RT that is not adjacent to any 
     *    of the species in specList, null if no such RT exists
     */
    ...
    /**
     * 
     * @return Return a set of essential rticators, when an rticator
     * is essential if it is not covered by other rticators
     */
     ...
     * After calling the method, the rticators in the list which is
     * 'adjacent' to the species are removed 
     */
